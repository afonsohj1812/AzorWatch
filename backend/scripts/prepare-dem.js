...
function buildIsland(tiles, island) {
  ...
  const dLat = CELL_SIZE / M_PER_DEG_LAT;
  const dLon = CELL_SIZE / (M_PER_DEG_LAT * Math.cos((latMid * Math.PI) / 180));
...
function deriveSlopeAspect(elevation, width, height) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const dzdE = (ne + 2 * e + se - (nw + 2 * w + sw)) / (8 * CELL_SIZE);
      const dzdN = (nw + 2 * n + ne - (sw + 2 * s + se)) / (8 * CELL_SIZE);
...
function verify(grids) {
  ...
  const areaKm2 =
    ([...grids.values()].reduce((sum, g) => sum + g.land, 0) * CELL_SIZE ** 2) /
    1e6;
  const areaOk =
...
async function main() {
  ...
  for (const island of islands) {
    ...
    console.log(
      `  ${island.id.padEnd(12)} ${grid.width}x${grid.height}  ${((grid.land * CELL_SIZE ** 2) / 1e6).toFixed(0)} km2 land`,
    );
  }
  ...
  if (failures) {
    console.error(
      `\n${failures} check(s) failed — not usable, fix before continuing`,
    );
    process.exit(1);
  }
  console.log("\nall checks passed");
